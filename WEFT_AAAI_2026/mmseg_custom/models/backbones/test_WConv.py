...
class GatingNetwork(nn.Module):
    def __init__(self, in_features, num_experts):  # ← 将 `experts` 改为 `num_experts`
        super(GatingNetwork, self).__init__()
        self.fc = nn.Linear(in_features, num_experts)  # 输出 num_experts 个专家的权重
        self.softmax = nn.Softmax(dim=1)

    def forward(self, x):
        x = self.fc(x)
        return self.softmax(x)  # 归一化为概率分布
...
class WTConv_Expert(nn.Module):
    ...
    def forward(self, x):
        ...
        gate_weights = self.gating_network(x_flattened.mean(dim=-1))  # 输出 (batch_size, 10)


        top4_weights, top4_indices = torch.topk(gate_weights, k=4, dim=1)  # (batch_size, 4)


        top4_weights = torch.softmax(top4_weights, dim=1) + 1e-8  # 避免 NaN
        top4_weights /= top4_weights.sum(dim=1, keepdim=True)  # 重新归一化
        ...
        return final_output  # (batch_size, C, H, W)
...
class Module1(nn.Module):
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            c1 = self.stem(x)
            c1 = c1 + self.WTConv_EH(c1)  # 4
            c2 = self.down_sampling(c1)  # 8
            c2 = c2 + self.WTConv_EH(c2)
            c3 = self.down_sampling(c2)  # 16
