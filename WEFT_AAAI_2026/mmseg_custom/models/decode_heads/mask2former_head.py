...
class Mask2FormerHead(BaseDecodeHead):
    ...
    def forward_test(self, inputs, img_metas=None, test_cfg=None):
        if img_metas is None:
            img_metas = [{'ori_shape': (inputs[0].shape[2], inputs[0].shape[3], inputs[0].shape[1])}]

        # 如果 ori_shape 仍然缺少通道数，默认补充为 3
        for meta in img_metas:
            ori_shape = meta['ori_shape']
            if len(ori_shape) == 2:  # 如果只有 (height, width)
                meta['ori_shape'] = (ori_shape[0], ori_shape[1], 3)  # 假设是 RGB 图像
